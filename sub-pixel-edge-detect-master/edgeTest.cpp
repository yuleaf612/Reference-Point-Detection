...
{
	cv::Mat mModG = cv::Mat(Y, X, CV_64F, cv::Scalar(0));
	cv::Mat mGX = cv::Mat(Y, X, CV_64F, cv::Scalar(0));
	cv::Mat mGY = cv::Mat(Y, X, CV_64F, cv::Scalar(0));
	...
	for (y = 2; y<(Y - 2); y++)
	{
		...
		if (Dx > 0 || Dy > 0)
		{
			...
			mGX.at<double>(y, x) = Ex[x + y*X];
			mGY.at<double>(y, x) = Ey[x + y*X];
		}
	}
}
